# Air ref      - complete pointwise division
# Spatial norm - divide spatially , with a certain spatial profile (for now wavelength)
...
if __name__ == '__main__':
    ...
    x15_rt_processed_path = os.path.join(x15_rt_processed_path , 'x15_referencing')
    if not os.path.exists(x15_rt_processed_path):
        os.mkdir(x15_rt_processed_path)
    ...
    for i in range(0,n_children):
        ...
        for j in range(0,len(unique_configurations)):
            ...
            air_ref_plots_folder = os.path.join(config_folder , 'normalized')
                    ...
                    if ab:
                        ...
                        for j in range(0,len(skeletal_ureter_sublist)):
                            ...
                            if not current_content['incomplete_data']:
                                ...
                                current_referenced = np.zeros(np.shape(current_normalized))
                                for k in range(0,15):
                                    if k < 11:
                                        current_referenced[k,:] = np.divide(current_normalized[k,:],current_normalized[4,:])
                                    else:
                                        current_referenced[k,:] = np.divide(current_normalized[k,:],current_normalized[14,:])
                                current_referenced = current_referenced/np.max(current_referenced)

                                legend.append(current_meta['pig1'] + '-' + current_meta['spec1'] + '-' + current_meta['sample1'])
                                plt.plot(wl_set,current_referenced[:,skeletal_ureter_rois[j][2]])
                        
                        if vertical_lines:
                            for j in range(0,len(wl_set)):
                                plt.axvline(wl_set[j])
                        plt.legend(legend)
                        ure_norm_save_at = os.path.join(air_ref_plots_folder,'ure_centre_pixel_ab.pdf')
                    ...
                    if ba:
                        ...
                        for j in range(0,len(skeletal_ureter_sublist)):
                            ...
                            if not current_content['incomplete_data']:
                                ...
                                reference_std = np.zeros(np.shape(current_content['x15_c']))
                                sample_std    = np.zeros(np.shape(current_content['x15_c']))
                                for k in range(0,15):
                                    if k < 11:
                                        reference_std[k,:] = np.divide(reference_read[k,:],reference_read[4,:])
                                        sample_std[k,:]    = np.divide(current_content['x15_c'][k,:],current_content['x15_c'][4,:])
                                    else:
                                        reference_std[k,:] = np.divide(reference_read[k,:],reference_read[14,:])
                                        sample_std[k,:]    = np.divide(current_content['x15_c'][k,:],current_content['x15_c'][14,:])


                                current_normalized = np.divide(sample_std,reference_std)
                        ...
                        if vertical_lines:
                            for j in range(0,len(wl_set)):
                                plt.axvline(wl_set[j])
                        plt.legend(legend)
                        ure_norm_save_at = os.path.join(air_ref_plots_folder,'ure_centre_pixel_ba.pdf')
                        plt.savefig(ure_norm_save_at)
                    ...
                    if ab:
                        ...
                        for j in range(0,len(peritoneum_sublist)):
                            ...
                            if not current_content['incomplete_data']:
                                ...
                                legend.append(current_meta['pig1'] + '-' + current_meta['spec1'] + '-' + current_meta['sample1'])
                                plt.plot(wl_set,current_referenced[:,peritoneum_rois[j][2]])
            
                        if vertical_lines:
                            for j in range(0,len(wl_set)):
                                plt.axvline(wl_set[j])
                        plt.legend(legend)
                        peri_norm_save_at = os.path.join(air_ref_plots_folder,'peri_centre_pixel_ab.pdf')
                    ...
                    if ba:
                        ...
                        if vertical_lines:
                            for j in range(0,len(wl_set)):
                                plt.axvline(wl_set[j])
                        plt.legend(legend)
                        peri_norm_save_at = os.path.join(air_ref_plots_folder,'peri_centre_pixel_ba.pdf')
                        plt.savefig(peri_norm_save_at)
                    ...
                    if ab:
                        ...
                        for j in range(0,len(native_ureter_sublist)):
                            ...
                            if not current_content['incomplete_data']:
                                ...
                                legend.append(current_meta['pig1'] + '-' + current_meta['spec1'] + '-' + current_meta['sample1'])
                                plt.plot(wl_set,current_referenced[:,native_ureter_rois[j][2]])
                        
                        if vertical_lines:
                            for j in range(0,len(wl_set)):
                                plt.axvline(wl_set[j])
                        plt.legend(legend)
                        ure_norm_save_at = os.path.join(air_ref_plots_folder,'native_ure_centre_pixel_ab.pdf')
                        plt.savefig(ure_norm_save_at)
                    ...
                    if ba:
                        ...
                        if vertical_lines:
                            for j in range(0,len(wl_set)):
                                plt.axvline(wl_set[j])
                        plt.legend(legend)
                        ure_norm_save_at = os.path.join(air_ref_plots_folder,'native_ure_centre_pixel_ba.pdf')
                        plt.savefig(ure_norm_save_at)
                #plot air reference normalized values.
            #Plot native ureters.