...
if __name__ == '__main__':
    ...
    x15_rt_processed_path = os.path.join(x15_rt_processed_path , 'x15_baseline_analysis')
    if not os.path.exists(x15_rt_processed_path):
        os.mkdir(x15_rt_processed_path)
    ...
    for i in range(0,n_children):
        ...
        for j in range(0,n_targets):
            ...
            current_configuration = 'ja_' + current_jaw_angle + '_s1_' + current_s1 + '_pos1_' + current_position1 + '_s2_' + current_s2 + '_pos2_' + current_position2
            
            if current_tissue1 == 'ureter' and current_tissue2 == 'peritoneum':
                
                if current_configuration not in unique_ure_configurations:
                    unique_ure_configurations.append(current_configuration)

            if current_tissue1 == 'peritoneum' and current_tissue2 == 'none':
        ...
        if n_ure_configs:
            ...
            for config in ure_configs:
                ...
                if config not in ['n_configs']:
                    ...
                    baselines = [int(process_x15_rt_file_name(data_list[k])['baseline']) for k in range(0,n_data)]
                    #data      = np.array([process_file_content_x15_rt_two_step(data_list[k])['x15_c']  for k in range(0,n_data)])
                    baselines = []
                    cmos      = []
                    for k in range(0,n_data):
                        baseline = int(process_x15_rt_file_name(data_list[k])['baseline'])
